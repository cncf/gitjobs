    ...
    return html`
      <div class="mt-2 relative location-container">
        <div class="absolute inset-y-0 rtl:inset-r-0 start-0 flex items-center ps-3 pointer-events-none">
          <div
            class="svg-icon bg-stone-300 ${
              this.size === "small" ? " size-3.5 icon-location" : "size-4 icon-search"
            }"
          ></div>
        </div>
        <input
          ...
          type="text"
          .value=${this.value}
          class="input-primary peer ${
            this.size === "small" ? "py-0.5 peer px-9 rounded-lg text-[0.775rem]/6 text-stone-700" : "px-10"
          }"
        ...
        <div class="absolute end-1.5 top-1.5 peer-placeholder-shown:hidden">
          <button @click=${this._cleanInput} type="button" class="cursor-pointer mt-[2px]">
            <div
              class="svg-icon bg-stone-400 hover:bg-stone-700 icon-close ${
                this.size === "small" ? "size-4" : "size-5"
              }"
            ></div>
          </button>
        </div>
